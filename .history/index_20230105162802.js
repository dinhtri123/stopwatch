...
const replaceClass = (classOld, classNew) => {
  return iconStart.className.replace(classOld, classNew);
};
...
elm(".btn-start").addEventListener("click", function () {
  ...
  interval = setInterval(() => {
    ...
    elm(".miliseconds").textContent =
      cusTime(milis) > 99 ? (milis = "0" + 0) : cusTime(milis);
    if (cusTime(milis) === 100) {
      sec++;
      elm(".seconds").textContent = cusTime(sec) > 59 ? (sec = "0" + 0) : cusTime(sec);
      if (cusTime(sec) === 60) {
        min++;
        if (cusTime(min) === 60) {
          hrs++;
  ...
  if (iconStart.className.includes("fa-play")) {
    iconStart.className = replaceClass("fa-play", "fa-pause");
    this.disabled = true;
    this.classList.add("btn-disabled");
    elm(".btn-stop").classList.add("btn-enabled");
    elm(".btn-stop").disabled = false;
  } else {
    iconStart.className = replaceClass("fa-pause", "fa-play");
  }
});
