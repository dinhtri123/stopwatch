...
btnReset.addEventListener("click", function () {
  ...
  localStorage.setItem("timer", JSON.stringify(storageTimer));
  ...
  const getTimerStorage = JSON.parse(localStorage.getItem("timer"));
  for (let i = 0; i < getTimerStorage.length; i++) {
    for (let j = 0; j < getTimerStorage[i].length; j++) {
      ...
      <div class="timer-lap">
        <p>${getTimerStorage[i][0]}</p>
        <span>:</span>
        <p>${getTimerStorage[i][1]}</p>
        <span>:</span>
        <p>${getTimerStorage[i][2]}</p>
        <span>.</span>
        <p>${getTimerStorage[i][3]}</p>
      </div>
    </div>`;
